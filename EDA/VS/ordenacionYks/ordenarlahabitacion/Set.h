...
public:
    ...
    //Esta búsqueda es de n, lineal
    bool operator<=(const Set<T>& scndSet) const{

        if (empty()) return true;

        int set1pt = 0, set2pt = 0;
        int foundNo = 0;

        while (set1pt < nelems && set2pt < scndSet.nelems) {
            
            if (array[set1pt] == scndSet.array[set2pt]) {
                set1pt++;
                set2pt++;
                foundNo++;
            }

            else if (array[set1pt] > scndSet.array[set2pt]) set2pt++;

            else break;
        }

        return foundNo == nelems;
    }
